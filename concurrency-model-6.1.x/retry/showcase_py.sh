...
printf "${CYAN}"
tailgrep -n10 logs/ncs-python-vm-server-config-python.log -m1 "DNS service - try acquiring the semaphore"
printf "${NC}"
...
printf "\n${PURPLE}##### Once the NTP service finishes, the DNS service will retry, followed by an NTP service retry. See the NTP service Python code for why it too retries\n${NC}"
...
printf "\n\n${PURPLE}##### Print the contents of the Python 'server' service log for finished create callbacks\n${NC}"
printf "${CYAN}"
tailgrep -n10 logs/ncs-python-vm-server-config-python.log -m4 "Service create done"
printf "${NC}"
...
printf "\n\n${GREEN}##### Showcase: Trigger conflicts handled using the NSO Python API for the retry mechanism\n${NC}"
...
printf "${CYAN}"
tailgrep -n10 logs/ncs-python-vm-server-config-python.log -m1 "Update NTP without retry action - try acquiring the semaphore"
printf "${NC}"
...
printf "\n\n${PURPLE}##### Release the semaphore to allow the 'update-ntp' action Python program to finish and NSO to detect the conflict and abort the transaction\n${NC}"
ncs_cli -n -C -u admin << EOF
servers release-sem
EOF

printf "\n${CYAN}"
tailgrep -n10 logs/ncs-python-vm-server-config-python.log -m1 "Conflict detected"
cat logs/devel.log | grep -A3 "conflict on"
printf "${NC}"
...
printf "${CYAN}"
tailgrep -n10 logs/ncs-python-vm-server-config-python.log -m1 "Update NTP with retry action - try acquiring the semaphore"
printf "${NC}"
...
printf "\n\n${PURPLE}##### Release the semaphore to allow the 'update-ntp-retry' action Python program to finish. NSO will detect the conflict and call the retry function\n${NC}"
ncs_cli -n -C -u admin << EOF
servers release-sem
EOF

printf "\n${CYAN}"
tailgrep -n10 logs/ncs-python-vm-server-config-python.log -m2 "Update NTP with retry action - done" | tail -1
...
printf "\n\n${PURPLE}##### Yet again lock the server list using the action that acquires the semaphore\n${NC}"
ncs_cli -n -C -u admin << EOF
servers acquire-sem
EOF

printf "\n\n${PURPLE}##### Create an NTP server instance using an action ${RED}*with decorator* retry${PURPLE} - will succeed after a retry\n${NC}"
echo "servers update-ntp-retry-dec device ex0 server server1" | ncs_cli -C -u admin &
echo "NTP server instance is being created"

printf "${CYAN}"
tailgrep -n10 logs/ncs-python-vm-server-config-python.log -m1 "Update NTP with decorator retry action - try acquiring the semaphore"
printf "${NC}"

printf "\n${PURPLE}##### Update the dual purpose DNS NTP server list while while the 'update-ntp-retry-dec' action is waiting for the semaphore to cause a conflict\n${NC}"
ncs_cli -n -C -u admin << EOF
config
servers server server1 ip 33.33.33.33
commit
EOF

printf "\n\n${PURPLE}##### Release the semaphore to allow the 'update-ntp-retry-dec' action Python program to finish. NSO will detect the conflict and call the decorator retry function\n${NC}"
ncs_cli -n -C -u admin << EOF
servers release-sem
EOF

printf "\n${CYAN}"
tailgrep -n10 logs/ncs-python-vm-server-config-python.log -m1 "Update NTP with retry decorator action - done"
cat logs/devel.log | grep -A3 "conflict on" | tail -4
printf "${NC}"
...
printf "\n\n${GREEN}##### Showcase done: Python API retry\n${NC}"
